...
class SingleThreadedStateMachineManager(
    ...
    override fun start(tokenizableServices: List<Any>) {
        ...
        logger.info("binhnt: SingleThreadedStateMachineManager.start: call checkQuasarJavaAgentPresence")
        checkQuasarJavaAgentPresence()

        logger.info("binhnt: SingleThreadedStateMachineManager.start: call CheckpointSerializationDefaults")
        ...
        logger.info("binhnt: SingleThreadedStateMachineManager.start: set checkpointSerializationContext")
        this.checkpointSerializationContext = checkpointSerializationContext

        logger.info("binhnt: SingleThreadedStateMachineManager.start: makeActionExecutor")
        this.actionExecutor = makeActionExecutor(checkpointSerializationContext)

        logger.info("binhnt: SingleThreadedStateMachineManager.start: checkpointSerializationContext")
        fiberDeserializationChecker?.start(checkpointSerializationContext)

        logger.info("binhnt: SingleThreadedStateMachineManager.start: restoreFlowsFromCheckpoints")
        ...
        Fiber.setDefaultUncaughtExceptionHandler { fiber, throwable ->
            if (throwable is VirtualMachineError) {
                ...
                logger.info("binhnt: SingleThreadedStateMachineManager.start: set fiber as FlowStateMachineImpl ")
                (fiber as FlowStateMachineImpl<*>).logger.warn("Caught exception from flow", throwable)
            }
        }
        serviceHub.networkMapCache.nodeReady.then {
            logger.info("binhnt: SingleThreadedStateMachineManager.start: serviceHub.networkMapCache nodeReady")
            logger.info("Node ready, info: ${serviceHub.myInfo}")

            logger.info("binhnt: SingleThreadedStateMachineManager.start: resumeRestoredFlows from fibers")
            resumeRestoredFlows(fibers)

            logger.info("binhnt: SingleThreadedStateMachineManager.start: call flowMessaging and deliverExternalEvent")
    ...
    override fun <A : FlowLogic<*>> findStateMachines(flowClass: Class<A>): List<Pair<A, CordaFuture<*>>> {
        logger.info("binhnt: SingleThreadedStateMachineManager.findStateMachines: start")
        return mutex.locked {
    ...
    override fun stop(allowedUnsuspendedFiberCount: Int) {
        logger.info("binhnt: SingleThreadedStateMachineManager.stop: start")
    ...
    private fun <A> startFlow(
        ...
        logger.info("binhnt: SingleThreadedStateMachineManager.startFlow: call startFlowInternal ")
    ...
    override fun killFlow(id: StateMachineRunId): Boolean {
        logger.info("binhnt: SingleThreadedStateMachineManager.killFlow: mutex.locked: database.transaction ")
    ...
    override fun addSessionBinding(flowId: StateMachineRunId, sessionId: SessionId) {
        logger.info("binhnt: SingleThreadedStateMachineManager.addSessionBinding: start ")
    ...
    override fun deliverExternalEvent(event: ExternalEvent) {
        logger.info("binhnt: SingleThreadedStateMachineManager.deliverExternalEvent: lock thread and call e ")
        mutex.locked {
            if (!stopping) {
                when (event) {
    ...
    private fun <T> onExternalStartFlow(event: ExternalEvent.ExternalStartFlowEvent<T>) {
        logger.info("binhnt: SingleThreadedStateMachineManager.onExternalStartFlow: get event $event and start flow  ")
    ...
    private fun onSessionMessage(event: ExternalEvent.ExternalMessageEvent) {
        ...
        logger.info("binhnt: SingleThreadedStateMachineManager.onSessionMessage: session message ")

        val peer = event.receivedMessage.peer

        logger.info("binhnt: SingleThreadedStateMachineManager.onSessionMessage: deserialize message to get sesionMessage ")
    ...
    private fun onExistingSessionMessage(sessionMessage: ExistingSessionMessage, deduplicationHandler: DeduplicationHandler, sender: Party) {
        logger.info("binhnt: SingleThreadedStateMachineManager.onExistingSessionMessage: start ")
        try {

            logger.info("binhnt: SingleThreadedStateMachineManager.onExistingSessionMessage: get recipientId from sessionMesssage  ")
            val recipientId = sessionMessage.recipientSessionId

            logger.info("binhnt: SingleThreadedStateMachineManager.onExistingSessionMessage: get flow id  ")
            val flowId = sessionToFlow[recipientId]
            if (flowId == null) {
                logger.info("binhnt: SingleThreadedStateMachineManager.onExistingSessionMessage: call deduplicationHandler.afterDatabaseTransaction ")
                ...
                logger.info("binhnt: SingleThreadedStateMachineManager.onExistingSessionMessage: try to lock $flowId ")
                val flow = mutex.locked { flows[flowId] }
                        ?: throw IllegalStateException("Cannot find fiber corresponding to ID $flowId")

                logger.info("binhnt: SingleThreadedStateMachineManager.onExistingSessionMessage:set  flow.fiber.scheduleEvent to run  Event.DeliverSessionMessage ")
    ...
    private fun onSessionInit(sessionMessage: InitialSessionMessage, sender: Party, event: ExternalEvent.ExternalMessageEvent) {
        logger.info("binhnt: SingleThreadedStateMachineManager.onSessionInit: start initialize session ")
        try {
            logger.info("binhnt: SingleThreadedStateMachineManager.onSessionInit: get initiatedFlowFactory from sessionMessage")
            val initiatedFlowFactory = getInitiatedFlowFactory(sessionMessage)

            logger.info("binhnt: SingleThreadedStateMachineManager.onSessionInit: create initiatedSessionId")
            val initiatedSessionId = SessionId.createRandom(secureRandom)

            logger.info("binhnt: SingleThreadedStateMachineManager.onSessionInit: create FlowSessionImpl")
            val senderSession = FlowSessionImpl(sender, initiatedSessionId)

            logger.info("binhnt: SingleThreadedStateMachineManager.onSessionInit: create flowLogic")
            val flowLogic = initiatedFlowFactory.createFlow(senderSession)

            logger.info("binhnt: SingleThreadedStateMachineManager.onSessionInit: get FlowInfo")
            ...
            logger.info("binhnt: SingleThreadedStateMachineManager.onSessionInit: get senderCoreFlowVersion")
            ...
            logger.info("binhnt: SingleThreadedStateMachineManager.onSessionInit: call startInitiatedFlow")
    ...
    private fun getInitiatedFlowFactory(message: InitialSessionMessage): InitiatedFlowFactory<*> {
        logger.info("binhnt: SingleThreadedStateMachineManager.getInitiatedFlowFactory: start ")

        logger.info("binhnt: SingleThreadedStateMachineManager.getInitiatedFlowFactory: call initiatorClass ")
        ...
        logger.info("binhnt: SingleThreadedStateMachineManager.getInitiatedFlowFactory: call initiatorFlowClass ")
    ...
    private fun <A> startInitiatedFlow(
        ...
        logger.info("binhnt: SingleThreadedStateMachineManager.startInitiatedFlow: create flowStart ")
        ...
        logger.info("binhnt: SingleThreadedStateMachineManager.startInitiatedFlow: call  startFlowInternal")
        startFlowInternal(
    ...
    private fun <A> startFlowInternal(
        ...
        logger.info("binhnt: SingleThreadedStateMachineManager.startFlowInternal: flowId = $flowId ")
        ...
        logger.info("binhnt: SingleThreadedStateMachineManager.startFlowInternal: create flowStateMachineImpl")
        val flowStateMachineImpl = FlowStateMachineImpl(flowId, flowLogic, scheduler)
        val resultFuture = openFuture<Any?>()

        logger.info("binhnt: SingleThreadedStateMachineManager.startFlowInternal: update flowStateMachineImpl.transientValues ")
        flowStateMachineImpl.transientValues = TransientReference(createTransientValues(flowId, resultFuture))

        logger.info("binhnt: SingleThreadedStateMachineManager.startFlowInternal: update flowLogic.stateMachine  ")
        flowLogic.stateMachine = flowStateMachineImpl

        logger.info("binhnt: SingleThreadedStateMachineManager.startFlowInternal: get checkpointSerialize ")
        val frozenFlowLogic = (flowLogic as FlowLogic<*>).checkpointSerialize(context = checkpointSerializationContext!!)

        logger.info("binhnt: SingleThreadedStateMachineManager.startFlowInternal: createSubFlowVersion")
        ...
        logger.info("binhnt: SingleThreadedStateMachineManager.startFlowInternal: create initialCheckpoint")
        ...
        logger.info("binhnt: SingleThreadedStateMachineManager.startFlowInternal: create StateMachineState")
        ...
        logger.info("binhnt: SingleThreadedStateMachineManager.startFlowInternal: set flowStateMachineImpl.transientState")
        ...
        logger.info("binhnt: SingleThreadedStateMachineManager.startFlowInternal: increate totalStartedFlows")

        totalStartedFlows.inc()

        logger.info("binhnt: SingleThreadedStateMachineManager.startFlowInternal: call addAndStartFlow")
    ...
    private fun createFlowFromCheckpoint(
        ...
        logger.info("binhnt: SingleThreadedStateMachineManager.createFlowFromCheckpoint: start ")
    ...
    private fun addAndStartFlow(id: StateMachineRunId, flow: Flow) {
        logger.info("binhnt: SingleThreadedStateMachineManager.addAndStartFlow: start ")

        val checkpoint = flow.fiber.snapshot().checkpoint

        logger.info("binhnt: SingleThreadedStateMachineManager.addAndStartFlow: get list of session ")
        for (sessionId in getFlowSessionIds(checkpoint)) {
            sessionToFlow[sessionId] = id
        }

        logger.info("binhnt: SingleThreadedStateMachineManager.addAndStartFlow: lock thread ")
        mutex.locked {
            if (stopping) {
                ...
                logger.info("binhnt: SingleThreadedStateMachineManager.addAndStartFlow: Not resuming as SMM is stopping. ")

            } else {
                logger.info("binhnt: SingleThreadedStateMachineManager.addAndStartFlow: put flow in flows list ")
                val oldFlow = flows.put(id, flow)
                if (oldFlow == null) {
                    logger.info("binhnt: SingleThreadedStateMachineManager.addAndStartFlow: incrementLiveFibers")
                    incrementLiveFibers()
                    unfinishedFibers.countUp()
                } else {
                    logger.info("binhnt: SingleThreadedStateMachineManager.addAndStartFlow: oldFlow.resultFuture.captureLater")
                    oldFlow.resultFuture.captureLater(flow.resultFuture)
                }

                logger.info("binhnt: SingleThreadedStateMachineManager.addAndStartFlow: get flowLogic from  FlowStateMachineImpl ")
                val flowLogic = flow.fiber.logic

                logger.info("binhnt: SingleThreadedStateMachineManager.addAndStartFlow: scheduleTimeoutl ")
                if (flowLogic.isEnabledTimedFlow()) scheduleTimeout(id)

                logger.info("binhnt: SingleThreadedStateMachineManager.addAndStartFlow: scheduleEvent ")
                flow.fiber.scheduleEvent(Event.DoRemainingWork)
                when (checkpoint.flowState) {
                    is FlowState.Unstarted -> {
                        logger.info("binhnt: SingleThreadedStateMachineManager.addAndStartFlow: fiber.start ")
                        flow.fiber.start()
                    }
                    is FlowState.Started -> {
    ...
    private fun makeActionExecutor(checkpointSerializationContext: CheckpointSerializationContext): ActionExecutor {
        logger.info("binhnt: SingleThreadedStateMachineManager.makeActionExecutor: start ")
