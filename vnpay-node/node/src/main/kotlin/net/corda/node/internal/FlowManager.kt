...
open class NodeFlowManager(flowOverrides: FlowOverrideConfig? = null) : FlowManager {
    ...
    override fun getFlowFactoryForInitiatingFlow(initiatedFlowClass: Class<out FlowLogic<*>>): InitiatedFlowFactory<*>? {
        log.info("binhnt: NodeFlowManager.getFlowFactoryForInitiatingFlow: get flowfactory")
    ...
    override fun <F : FlowLogic<*>> registerInitiatedFlow(responder: Class<F>) {
        log.info("binhnt: NodeFlowManager.registerInitiatedFlow: get registerInitiatedFlow with responder")
    ...
    override fun <F : FlowLogic<*>> registerInitiatedFlow(initiator: Class<out FlowLogic<*>>, responder: Class<F>) {
        log.info("binhnt: NodeFlowManager.registerInitiatedFlow: get registerInitiatedFlow with initiator, responder")
        val constructors = responder.declaredConstructors.associateBy { it.parameterTypes.toList() }
        log.info("binhnt: NodeFlowManager.registerInitiatedFlow: get contructor with flowsession class ")
        ...
        val ctor: (FlowSession) -> F = if (flowSessionCtor == null) {
            ...
            log.info("binhnt: NodeFlowManager.registerInitiatedFlow: get contructor with Party class ")
        ...
        log.info("binhnt: NodeFlowManager.registerInitiatedFlow: call InitiatedFlowFactory.CorDapp")
        val flowFactory = InitiatedFlowFactory.CorDapp(version, responder.appName, ctor)

        log.info("binhnt: NodeFlowManager.registerInitiatedFlow: call registerInitiatedFlowFactory")
    ...
    private fun <F : FlowLogic<*>> registerInitiatedFlowFactory(initiatingFlowClass: Class<out FlowLogic<*>>,
        ...
        log.info("binhnt: NodeFlowManager.registerInitiatedFlowFactory: start")
        ...
        synchronized(listOfFlowsForInitiator) {
            log.info("binhnt: NodeFlowManager.registerInitiatedFlowFactory: call RegisteredFlowContainer")
