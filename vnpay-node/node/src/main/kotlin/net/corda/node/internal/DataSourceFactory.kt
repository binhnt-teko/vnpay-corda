...
object DataSourceFactory {
    ...
    fun createDataSource(hikariProperties: Properties, pool: Boolean = true, metricRegistry: MetricRegistry? = null): DataSource {
        println("binhnt: DataSourceFactory.createDataSource: create hikari config")
        val config = HikariConfig(hikariProperties)

        println("binhnt: DataSourceFactory.createDataSource: create datasource by pool or not")
        return if (pool) {
            println("binhnt: DataSourceFactory.createDataSource: create datasource by pool")
            val dataSource = HikariDataSource(config)

            println("binhnt: DataSourceFactory.createDataSource: check metricRegistry ")
            if (metricRegistry != null) {
                dataSource.metricRegistry = metricRegistry
            }
            dataSource
        } else {
            println("binhnt: DataSourceFactory.createDataSource: create one instance of db  ")
