...
open class NodeStartupCli : CordaCliWrapper("corda", "Runs a Corda Node") {
    ...
    //binhnt: get command options
    val cmdLineOptions = NodeCmdLineOptions()



    //binhnt: define list of sub commands
    ...
    override fun runProgram(): Int {
        //binhnt: Check each subcommand and run

        println("binhnt: NodeStartupCli.runProgram")
...
open class NodeStartup : NodeStartupLogging {
    ...
    //binhnt: this function used to call from outsite to run node and callback: afterNodeInitialisation
    fun initialiseAndRun(cmdLineOptions: SharedNodeCmdLineOptions, afterNodeInitialisation: RunAfterNodeInitialisation): Int {
        ...
        if (attempt {
                    ...
                    //binhnt: create node with nodeinfo
                    logger.info("binhnt: NodeStartup.initialiseAndRun: create node with $configuration and versionInfo and call  afterNodeInitialisation")
    ...
    //binhnt: this function startNode
    fun startNode(node: Node, startTime: Long) {
        logger.info("Binhnt: NodeStartup.startNode")
        //binhnt: check running mode
        ...
        //binhnt: check node info
        logger.info("Binhnt: NodeStartup.startNode: node.start")
        val nodeInfo = node.start()

        //binhnt: Load codapps
        logger.info("Binhnt: NodeStartup.startNode: load codapp")
        val loadedCodapps = node.services.cordappProvider.cordapps.filter { it.isLoaded }

        //binhnt: logLoadedCorDapps
        logger.info("Binhnt: NodeStartup.startNode: logLoadedCorDapps codapp")
        logLoadedCorDapps(loadedCodapps)

        logger.info("Binhnt: NodeStartup.startNode: node.nodeReadyFuture.thenMatch")
        node.nodeReadyFuture.thenMatch({
            ...
            //binhnt: start local shell server

            logger.info("Binhnt: nodeReadyFuture. Check node.configuration.shouldStartLocalShell")
            if (node.configuration.shouldStartLocalShell()) {
                logger.info("Binhnt: NodeStartup.startNode: nodeReadyFuture:  StartLocalShell")
                node.startupComplete.then {
                    try {
                        logger.info("Binhnt: NodeStartup.startNode: try run runLocalShell when node.startupComplete")
            ...
            //binhnt: start ssh server
            logger.info("Binhnt: nodeReadyFuture. Check and start ssh daemon")
        ...
        //binhnt: node.run: process message from queue
        node.run()
    }
