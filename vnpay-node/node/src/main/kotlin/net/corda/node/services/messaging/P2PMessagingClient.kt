...
class P2PMessagingClient(val config: NodeConfiguration,
    ...
    fun start(myIdentity: PublicKey, serviceIdentity: PublicKey?, maxMessageSize: Int, advertisedAddress: NetworkHostAndPort = serverAddress) {
        log.info("Binhnt: P2PMessagingClient.start: start client of p2p network ")
        ...
        state.locked {
            ...
            log.info("Binhnt: P2PMessagingClient.start: create message client ")
            ...
            log.info("Binhnt: P2PMessagingClient.start: create producerSession ")
            producerSession = createNewSession()

            log.info("Binhnt: P2PMessagingClient.start: create bridgeSession ")
            bridgeSession = createNewSession()

            log.info("Binhnt: P2PMessagingClient.start: start  producerSession and bridgeSession")
            ...
            log.info("Binhnt: P2PMessagingClient.start: create createProducer from session ")
            ...
            log.info("Binhnt: P2PMessagingClient.start: create P2PMessagingConsumer ")
            p2pConsumer = P2PMessagingConsumer(inboxes, createNewSession, isDrainingModeOn, drainingModeWasChangedEvents)

            log.info("Binhnt: P2PMessagingClient.start: create MessagingExecutor with producerSession ")
            ...
            log.info("Binhnt: P2PMessagingClient.start: registerBridgeControl ")
    ...
    private fun sendBridgeControl(message: BridgeControl) {
        log.info("binhnt: P2PMessagingClient.sendBridgeControl:  send message to party ")
    ...
    private fun artemisToCordaMessage(message: ClientMessage): ReceivedMessage? {
        log.info("binhnt: P2PMessagingClient.artemisToCordaMessage: start")
        try {
    ...
    internal fun deliver(artemisMessage: ClientMessage) {
        log.info("binhnt: P2PMessagingClient.deliver: Convert artemis to Corda message")
    ...
    private fun deliver(msg: ReceivedMessage, artemisMessage: ClientMessage) {
        log.info("binhnt: P2PMessagingClient.deliver: process ReceivedMessage base on deliverTo ")
        state.checkNotLocked()
        val deliverTo = handlers[msg.topic]
        if (deliverTo != null) {
            try {
                log.info("binhnt: P2PMessagingClient.deliver: call deliverTo depend on HandlerRegistration and $(msg.topic)")
    ...
    private fun createQueueIfAbsent(queueName: String, session: ClientSession, exclusive: Boolean) {
        log.info("binhnt: P2PMessagingClient.createQueueIfAbsent:  $queueName")
        fun sendBridgeCreateMessage() {
            log.info("binhnt: P2PMessagingClient.createQueueIfAbsent:  sendBridgeCreateMessage")
            val keyHash = queueName.substring(PEERS_PREFIX.length)
            val peers = networkMap.getNodesByOwningKeyIndex(keyHash)
            for (node in peers) {
                log.info("binhnt: P2PMessagingClient.createQueueIfAbsent:  create bridge to ${node.addresses} ")
                val bridge = BridgeEntry(queueName, node.addresses, node.legalIdentities.map { it.name })

                log.info("binhnt: P2PMessagingClient.createQueueIfAbsent:  create bridge message ")
                val createBridgeMessage = BridgeControl.Create(config.myLegalName.toString(), bridge)

                log.info("binhnt: P2PMessagingClient.createQueueIfAbsent:  send create bridge message ")
    ...
    override fun addMessageHandler(topic: String, callback: MessageHandler): MessageHandlerRegistration {
        ...
        log.info("Binhnt: P2PMessagingClient.addMessageHandler: add message handler")
    ...
    override fun removeMessageHandler(registration: MessageHandlerRegistration) {
        log.info("Binhnt: P2PMessagingClient.removeMessageHandler: remove message handler")
