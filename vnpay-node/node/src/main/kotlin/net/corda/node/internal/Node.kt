...
open class Node(configuration: NodeConfiguration,
    ...
    //binhnt: create message service client
    ...
    //binhnt: create message service server
    override fun startMessagingService(rpcOps: RPCOps, nodeInfo: NodeInfo, myNotaryIdentity: PartyAndCertificate?, networkParameters: NetworkParameters) {
        ...
        //binhnt: start message brocker
        log.info("binhnt: Node.startMessagingService: start message brocker")
        ...
        //binhnt: start message brocker and rpc brocker
        log.info("binhnt: Node.startMessagingService: start BrokerAddresses")
        ...
        //binhnt: setup brigde control
        log.info("Binhnt: Node.startMessagingService: start BridgeControlListener")
        ...
        log.info("bnhnt: Node.startMessagingService: Start up the embedded MQ server")
        messageBroker?.apply {
            closeOnStop()
            start()
        }
        log.info("bnhnt: Node.startMessagingService: Start up rpcBroker")
        rpcBroker?.apply {
            closeOnStop()
            start()
        }
        log.info("bnhnt: Node.startMessagingService: Start up bridgeControlListener")
        ...
        log.info("bnhnt: Node.startMessagingService:  Start up the MQ clients")
        ...
        log.info("bnhnt: Node.startMessagingService:  Start up network")
        network.start(
    ...
    override fun startDatabase() {
        log.info("binhnt: Node.startDatabase: start database ")
        ...
        if (databaseUrl != null && databaseUrl.startsWith(h2Prefix)) {
            ...
            log.info("binhnt: Node.startDatabase: database is H2 => config ")
            ...
            if (effectiveH2Settings?.address != null) {
                ...
                log.info("binhnt: Node.startDatabase: get database name ")
                val databaseName = databaseUrl.removePrefix(h2Prefix).substringBefore(';')

                log.info("binhnt: Node.startDatabase: get baseDir ")
                val baseDir = Paths.get(databaseName).parent.toString()

                log.info("binhnt: Node.startDatabase: create TCP server ")
                ...
                val url = try {
                    log.info("binhnt: Node.startDatabase: try to start h2 server ")
        ...
        log.info("binhnt: Node.startDatabase: start db with  databaseUrl = $databaseUrl ")
        super.startDatabase()
    ...
    override fun start(): NodeInfo {
        //binhnt: init serialization
        log.info("binhnt: Node.start: initialiseSerialization")
        initialiseSerialization()
        ...
        nodeReadyFuture.thenMatch({
            serverThread.execute {
                log.info("binhnt: Node.start:  node ready. Call registerJmxReporter")

                //binhnt: send log to jmx reporter
...
//        }.build().start()
    ...
    fun run() {
        //binhnt:  Start up the rpc clients.
        log.info("binhnt: Node.run(): Start up the rpc clients")
        internalRpcMessagingClient?.start(rpcBroker!!.serverControl)

        log.info("binhnt: Node.run(): start connect to peer")
        //binhnt: start connect to peer
