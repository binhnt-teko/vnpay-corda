...
//                     """
        val selectJoinAndLock = """
                        update contract_cash_states css_old
                        set account = ?
                        from  (SELECT vs.transaction_id,
                                                    vs.output_index,
                                                    ccs.pennies,
                ...
                (if (onlyFromIssuerParties.isNotEmpty()) " AND ccs.issuer_key = ANY (?)" else "") +
                (if (withIssuerRefs.isNotEmpty()) " AND ccs.issuer_ref = ANY (?)" else "") +
                """   ) nested
                                where nested.total < ?
                                and css_old.transaction_id = nested.transaction_id
                                and css_old.output_index = nested.output_index
                RETURNING   nested.transaction_id,
                            nested.output_index,
                            nested.pennies,
                            (nested.pennies + nested.total) as total,
                            nested.lock_id
                """
            connection.prepareStatement(selectJoinAndLock).use { statement ->
                statement.setLong(1, 0L)
                statement.setString(2, amount.token.toString() )
                var paramOffset = 0
                if (notary != null) {
                ...
                log.info("binhnt: CashSelectionPostgreSQLImpl.executeQuery: statement =  " + statement.toString())
