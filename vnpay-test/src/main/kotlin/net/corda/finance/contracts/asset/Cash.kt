...
class Cash : OnLedgerAsset<Currency, Cash.Commands, Cash.State>() {
    ...
    data class State(
            ...
            //Add to distinct to other cash
            val account: Long

    ) : FungibleAsset<Currency>, QueryableState {
        constructor(deposit: PartyAndReference, amount: Amount<Currency>, owner: AbstractParty,account: Long)
                : this(Amount(amount.quantity, Issued(deposit, amount.token)), owner,account)
     ...
     */
    //binhnt: Issue with pennies: Long value -> Convert to Amount type -> call generateIssue(TransactionBuilder,Amount,AbstractParty, Account,Party)
    fun generateIssue(tx: TransactionBuilder, tokenDef: Issued<Currency>, pennies: Long, owner: AbstractParty, notary: Party)
            = generateIssue(tx, Amount(pennies, tokenDef), owner,0, notary)

    /**
     * Puts together an issuance transaction for the specified amount that starts out being owned by the given pubkey.
     */
    fun generateIssue(tx: TransactionBuilder, amount: Amount<Issued<Currency>>, owner: AbstractParty, account: Long, notary: Party)
            = generateIssue(tx, TransactionState(State(amount, owner,account), PROGRAM_ID, notary), Commands.Issue())
    ...
    override fun verify(tx: LedgerTransaction) {
        println("binhnt: Cash.verify: start")
    ...
    private fun verifyIssueCommand(inputs: List<State>,
        ...
        println("binhnt: Cash.verifyIssueCommand: start")
    ...
    companion object {
        ...
        fun generateSpend(services: ServiceHub,
            ...
            //println("binhnt: Cash: companion object: generateSpend ")
            ...
            //println("binhnt: Cash: companion object: generateSpend: cashSelection: ")
            val cashSelection = AbstractCashSelection.getInstance({ services.jdbcSession().metaData })

            //println("binhnt: Cash: companion object: generateSpend: get unconsumed cashstate ")
            ...
            println("binhnt: Cash: companion object: generateSpend: total time: $total_time ")
...
val Amount<Currency>.CASH: Cash.State get() = Cash.State(Amount(quantity, Issued(NULL_PARTY.ref(1), token)), NULL_PARTY,0)
/** An extension property that lets you get a cash state from an issued token, under the [NULL_PARTY] */
val Amount<Issued<Currency>>.STATE: Cash.State get() = Cash.State(this, NULL_PARTY,0)
