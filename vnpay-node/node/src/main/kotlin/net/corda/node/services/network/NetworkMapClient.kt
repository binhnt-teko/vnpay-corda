...
class NetworkMapClient(compatibilityZoneURL: URL, private val versionInfo: VersionInfo) {
    ...
    fun start(trustRoot: X509Certificate) {
        logger.info("NetworkMapClient.start: start")
        this.trustRoot = trustRoot
    }

    fun publish(signedNodeInfo: SignedNodeInfo) {
        logger.info("NetworkMapClient.publish: publish node info")
    ...
    fun getNetworkMap(networkMapKey: UUID? = null): NetworkMapResponse {
        logger.info("NetworkMapClient.getNetworkMap: get network map")
