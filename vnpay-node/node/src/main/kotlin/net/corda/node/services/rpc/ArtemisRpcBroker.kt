...
class ArtemisRpcBroker internal constructor(
    ...
    override fun start() {
        logger.info("Binhnt: ArtemisRpcBroker.start: Artemis RPC broker is starting")
    ...
    override fun stop() {
        logger.info("Binhnt: ArtemisRpcBroker.stop: Artemis RPC broker is stopping")
    ...
    private fun initialiseServer(): ActiveMQServer {
        logger.info("Binhnt: ArtemisRpcBroker.initialiseServer: create serverConfiguration")
        ...
        logger.info("Binhnt: ArtemisRpcBroker.initialiseServer: create createArtemisSecurityManager with $serverConfiguration ")
        val serverSecurityManager = createArtemisSecurityManager(serverConfiguration.loginListener)

        logger.info("Binhnt: ArtemisRpcBroker.initialiseServer: return  ActiveMQServerImpl ")
    ...
    private fun createArtemisSecurityManager(loginListener: LoginListener): ActiveMQJAASSecurityManager {
        logger.info("Binhnt: ArtemisRpcBroker.createArtemisSecurityManager: start")
        ...
        logger.info("Binhnt: ArtemisRpcBroker.createArtemisSecurityManager: return  ActiveMQJAASSecurityManager")
