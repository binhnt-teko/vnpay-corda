...
abstract class AbstractCashSelection {
    ...
    fun unconsumedCashStatesForSpending(services: ServiceHub,
        ...
        //println("binhnt: AbstractCashSelection.unconsumedCashStatesForSpending: get unconsumecahs state for spending")
    ...
    private fun attemptSpend(services: ServiceHub, amount: Amount<Currency>, lockId: UUID, notary: Party?, onlyFromIssuerParties: Set<AbstractParty>, withIssuerRefs: Set<OpaqueBytes>, stateAndRefs: MutableList<StateAndRef<Cash.State>>): Boolean {
        ...
        //println("binhnt: AbstractCashSelection.attemptSpend: get unconsumecahs state for spending")
     ...
     //   spendLock.withLock {
            ...
            try {
                ...
                println("binhnt: AbstractCashSelection.attemptSpend: call executeQuery")
                return executeQuery(connection, amount, lockId, notary, onlyFromIssuerParties, withIssuerRefs) { rs ->
                    ...
                    while (rs.next()) {
                        ...
                        log.info( "Binhnt. AbstractCashSelection.attemptSpend:  ROW: $rowLockId ($lockId): ${StateRef(txHash, index)} : $pennies ($totalPennies)" )
                    ...
                    if (success) {
                        ...
                        log.info("binhnt: Coin selection for $amount retrieved ${stateAndRefs.count()} states totalling $totalPennies pennies: $stateAndRefs")
                        ...
                        log.info("binhnt: Coin selection requested $amount but retrieved $totalPennies pennies with state refs: ${stateAndRefs.map { it.ref }}")
                    }
                    success
                }
