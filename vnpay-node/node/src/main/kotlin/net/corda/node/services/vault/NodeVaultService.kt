...
class NodeVaultService(
    ...
    override fun start() {
        log.info("binhnt: NodeVaultService.start:  call bootstrapContractStateTypes ")
        bootstrapContractStateTypes()

        log.info("binhnt: NodeVaultService.start:  Observable.subscribe: process update message ")
    ...
    private fun saveStates(session: Session, states: Map<StateRef, StateAndRef<ContractState>>) {
        ...
        log.info("binhnt: NodeVaultService.saveStates:   foreach state save ")
        ...
        states.forEach { stateAndRef ->
            ...
            log.info("binhnt: NodeVaultService.saveStates:   with a state call VaultSchemaV1.VaultStates ")
            ...
            log.info("binhnt: NodeVaultService.saveStates:   call session.save ")
    ...
    private fun recordUpdate(update: Vault.Update<ContractState>): Vault.Update<ContractState> {
        log.info("binhnt: NodeVaultService.recordUpdate:   start ")
        ...
        if (!update.isEmpty()) {
            ...
            // Persist the outputs.
            log.info("binhnt: NodeVaultService.recordUpdate:   saveStates: producedStateRefsMap ")
            saveStates(session, producedStateRefsMap)

            // Persist the reference states.
            log.info("binhnt: NodeVaultService.recordUpdate:   saveStates: referenceStateRefsMap ")
            saveStates(session, referenceStateRefsMap)

            // Persist the consumed inputs.
            log.info("binhnt: NodeVaultService.recordUpdate:   saveStates: consumedStateRefs ")
            consumedStateRefs.forEach { stateRef ->
                log.info("binhnt: NodeVaultService.recordUpdate:   consumedStateRefs: get state base on  PersistentStateRef(stateRef)")
                val state = session.get<VaultSchemaV1.VaultStates>(VaultSchemaV1.VaultStates::class.java, PersistentStateRef(stateRef))
                log.info("binhnt: NodeVaultService.recordUpdate:   consumedStateRefs: call state?.run  VaultSchemaV1.VaultStates ")
    ...
    override fun notifyAll(statesToRecord: StatesToRecord, txns: Iterable<CoreTransaction>) {
        log.info("binhnt: NodeVaultService.notifyAll:   start ")
    ...
    private fun makeUpdates(batch: Iterable<CoreTransaction>, statesToRecord: StatesToRecord): List<Vault.Update<ContractState>> {
        log.info("binhnt: NodeVaultService.makeUpdates:   start ")
    ...
    private fun loadStates(refs: Collection<StateRef>): Collection<StateAndRef<ContractState>> {
        log.info("binhnt: NodeVaultService.loadStates:   start ")
    ...
    private fun processAndNotify(updates: List<Vault.Update<ContractState>>) {
        ...
        log.info("binhnt: NodeVaultService.processAndNotify:   start ")
    ...
    override fun getTransactionNotes(txnId: SecureHash): Iterable<String> {
        ...
        log.info("binhnt: NodeVaultService.getTransactionNotes:   start ")
    ...
    override fun softLockReserve(lockId: UUID, stateRefs: NonEmptySet<StateRef>) {
        ...
        log.info("binhnt: NodeVaultService.softLockReserve:   start ")
        ...
        try {
            ...
            //binhnt: function with input as function(CriteriaUpdate, Predicate): Any
            fun execute(configure: Root<*>.(CriteriaUpdate<*>, Array<Predicate>) -> Any?) = criteriaBuilder.executeUpdate(session) { update ->
                log.info("binhnt: NodeVaultService.softLockReserve: execute internal: get transaction state  ")
                val persistentStateRefs = stateRefs.map { PersistentStateRef(it.txhash.bytes.toHexString(), it.index) }

                log.info("binhnt: NodeVaultService.softLockReserve: execute internal: get compositeKey ")
                val compositeKey = get<PersistentStateRef>(VaultSchemaV1.VaultStates::stateRef.name)

                log.info("binhnt: NodeVaultService.softLockReserve: execute internal: get stateRefsPredicate ")
                val stateRefsPredicate = criteriaBuilder.and(compositeKey.`in`(persistentStateRefs))

                log.info("binhnt: NodeVaultService.softLockReserve: execute internal: call function  configure(update, arrayOf(stateRefsPredicate))")
                configure(update, arrayOf(stateRefsPredicate))
            }

            val updatedRows = execute { update, commonPredicates ->
                log.info("binhnt: NodeVaultService.softLockReserve: update and array of  Predicate  ")
                ...
                log.info("binhnt: NodeVaultService.softLockReserve: get lockIdPredicate ")
                ...
                log.info("binhnt: NodeVaultService.softLockReserve: update lockid  ")
                ...
                log.info("binhnt: NodeVaultService.softLockReserve: update with where  ")
                update.where(stateStatusPredication, lockIdPredicate, *commonPredicates)
            }

            if (updatedRows > 0 && updatedRows == stateRefs.size) {
                log.info( "binhnt: NodeVaultService.softLockReserve: Reserving soft lock states for $lockId: $stateRefs" )

                FlowStateMachineImpl.currentStateMachine()?.hasSoftLockedStates = true
            } else {

                log.info( "binhnt: NodeVaultService.softLockReserve: create revert Locked " )

                // revert partial soft locks
                val revertUpdatedRows = execute { update, commonPredicates ->

                    log.info( "binhnt: NodeVaultService.softLockReserve: revertUpdatedRows: set lockID " )
                ...
                if (revertUpdatedRows > 0) {
                    log.info( "binhnt: NodeVaultService.softLockReserve: revertUpdatedRows partially soft locked states for $lockId\" " )
//                    log.trace { "Reverting $revertUpdatedRows partially soft locked states for $lockId" }
            ...
            log.error("""soft lock update error attempting to reserve states for $lockId and $stateRefs, error =
                    $e
                """)
    ...
    override fun softLockRelease(lockId: UUID, stateRefs: NonEmptySet<StateRef>?) {
        ...
        log.info("binhnt: NodeVaultService.softLockRelease:   start ")
