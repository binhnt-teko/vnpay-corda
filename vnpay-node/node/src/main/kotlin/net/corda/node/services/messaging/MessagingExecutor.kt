...
class MessagingExecutor(
    ...
    fun send(message: Message, target: MessageRecipients) {
        log.info("binhnt: MessagingExecutor.send: start send message to target")
    ...
    internal fun cordaToArtemisMessage(message: Message): ClientMessage? {
        log.info("binhnt: MessagingExecutor.cordaToArtemisMessage: convert corda to artemis message")
