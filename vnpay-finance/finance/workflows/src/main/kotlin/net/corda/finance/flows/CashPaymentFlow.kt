...
open class CashPaymentFlow(
    ...
    override fun call(): AbstractCashFlow.Result {
        ...
        logger.info("binhnt: CashPaymentFlow.call: start = ${System.currentTimeMillis()} ")

        progressTracker.currentStep = GENERATING_ID
        val recipientSession = initiateFlow(recipient)
        logger.info("binhnt: CashPaymentFlow.call: initiateFlow = ${System.currentTimeMillis()} ")

        recipientSession.send(anonymous)
        logger.info("binhnt: CashPaymentFlow.call: recipientSession.send = ${System.currentTimeMillis()} ")
        ...
        logger.info("binhnt: CashPaymentFlow.call: recipient = ${System.currentTimeMillis()} ")

        val generate_point = System.currentTimeMillis()
        logger.info("binhnt: CashPaymentFlow.call: generate ID = ${generate_point - start} ")

        progressTracker.currentStep = GENERATING_TX
        val builder = TransactionBuilder(notary = notary ?: serviceHub.networkMapCache.notaryIdentities.first())
        logger.info("binhnt: CashPaymentFlow.call: build transaction = ${System.currentTimeMillis()} ")
        ...
        logger.info("binhnt: CashPaymentFlow.call: after Cash.generateSpend = ${System.currentTimeMillis()} ")

        val transaction_gen = System.currentTimeMillis()
        logger.info("binhnt: CashPaymentFlow.call: create transaction = ${transaction_gen - generate_point} ")

        progressTracker.currentStep = SIGNING_TX
        logger.info("Signing transaction for: ${spendTX.lockId}")
        val tx = serviceHub.signInitialTransaction(spendTX, keysForSigning)

        logger.info("binhnt: CashPaymentFlow.call: after serviceHub.signInitialTransaction = ${System.currentTimeMillis()} ")

        val sign_gen = System.currentTimeMillis()
        logger.info("binhnt: CashPaymentFlow.call: self sign ID = ${sign_gen - transaction_gen} ")
        ...
        logger.info("binhnt: CashPaymentFlow.call: after sessionsForFinality = ${System.currentTimeMillis()} ")

        val notarised = finaliseTx(tx, sessionsForFinality, "Unable to notarise spend")

        logger.info("binhnt: CashPaymentFlow.call: after finaliseTx = ${System.currentTimeMillis()} ")


        val finalize = System.currentTimeMillis()
        logger.info("binhnt: CashPaymentFlow.call: finalize = ${finalize - sign_gen} ")
