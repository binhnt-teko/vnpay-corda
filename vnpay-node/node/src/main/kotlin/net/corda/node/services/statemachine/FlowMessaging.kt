...
class FlowMessagingImpl(val serviceHub: ServiceHubInternal): FlowMessaging {
    ...
    override fun start(onMessage: (ReceivedMessage, deduplicationHandler: DeduplicationHandler) -> Unit) {
        log.info("binhnt: FlowMessagingImpl.start: addMessageHandler to network services ")
    ...
    override fun sendSessionMessage(party: Party, message: SessionMessage, deduplicationId: SenderDeduplicationId) {
        log.info("binhnt: FlowMessagingImpl.sendSessionMessage: createMessage with topic: $sessionTopic  and $deduplicationId $message to party $party")
        ...
        log.info("binhnt: FlowMessagingImpl.sendSessionMessage: send message to address: $address")
    ...
    private fun SessionMessage.additionalHeaders(target: Party): Map<String, String> {
        ...
        log.info("binhnt: FlowMessagingImpl.SessionMessage.additionalHeaders: add header")
