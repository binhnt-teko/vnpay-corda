...
abstract class OnLedgerAsset<T : Any, C : CommandData, S : FungibleAsset<T>> : Contract {
    companion object {
        ...
        fun <S : FungibleAsset<T>, T : Any> generateIssue(tx: TransactionBuilder,
            ...
            println("binhnt: OnLedgerAsset.generateIssue: gennerate FungibleAsset. Check  input state is empty and ouput state is empty")
            check(tx.inputStates().isEmpty())
            check(tx.outputStates().map { it.data }.filterIsInstance(transactionState.javaClass).isEmpty())

            println("binhnt: OnLedgerAsset.generateIssue: gennerate FungibleAsset. check transactionState.data.amount.quantity > 0 ")
            require(transactionState.data.amount.quantity > 0)

            println("binhnt: OnLedgerAsset.generateIssue: transactionState.data.amount.token.issuer ")
            val at = transactionState.data.amount.token.issuer

            println("binhnt: OnLedgerAsset.generateIssue: get owningKey of issuer")
            val commandSigner = at.party.owningKey

            println("binhnt: OnLedgerAsset.generateIssue: add transactionState to output")
            tx.addOutputState(transactionState)

            println("binhnt: OnLedgerAsset.generateIssue: add command with commandSigner")
